...
void cmdPositionSetpoint(
  ...
  void cmdRollPitchYawrateThrustChanged(
    const mav_msgs::RollPitchYawrateThrust::ConstPtr& msg)
  {
    if(!m_isEmergency)
    {
      float roll = msg->roll * 180 / pi();
      float pitch = -(msg->pitch * 180 / pi());
      float yawrate = msg->yaw_rate;
      uint16_t thrust = std::min<uint16_t>(std::max<float>((msg->thrust.z / 4) * 410000.0f, 0.0), 60000);
  ...
  void run()
  {
    ...
    m_subscribeCmdAttitudeThrust = n.subscribe(m_tf_prefix + "/cmd_attitude_thrust", 1, &CrazyflieROS::cmdRollPitchYawrateThrustChanged, this);
...
private:
  ...
  ros::Subscriber m_subscribeCmdAttitudeThrust;
